...
import { 
  ...
  ShieldCheck, CheckCircle2, Users, Smartphone, Loader2, User
} from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    e.preventDefault();
    if (mobile.length < 10 || fullName.trim() === "") return;
    setLoading(true);
        ...
        <div className="relative z-10 flex-1 flex flex-col">
          ...
          <div className="max-w-lg mt-auto lg:mt-24 mb-16">
            <h1 className="font-display font-black text-4xl lg:text-6xl text-[#1E4A32] leading-[1.1] mb-6">
              Join the future<br />of farming
              <span className="inline-block ml-3 text-3xl">🌾</span>
            </h1>
            <p className="text-lg text-[#3A5243] font-medium leading-relaxed max-w-md">
              Create an account to get personalized weather, schemes, and expert advisory directly for your crops.
            </p>
          </div>
      ...
      <div className="w-full lg:w-[45%] xl:w-[40%] flex flex-col bg-[#F9F7EF] lg:bg-transparent">
        ...
        <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
          ...
          <div className="w-full max-w-md bg-white rounded-[2rem] p-8 lg:p-10 shadow-2xl border border-gray-100 relative">
            ...
            >
              <Users size={14} /> Skip Sign Up <span className="text-[10px]">›</span>
            </button>

            <div className="text-center mt-6 mb-8">
              <h2 className="text-2xl font-display font-bold text-[#1E4A32] mb-2">Create an account</h2>
              <p className="text-sm text-gray-500 font-medium">Join KisanNet today</p>
            ...
            {step === 1 ? (
              <form onSubmit={handleSendOTP} className="space-y-5 animate-fadeIn">
                ...
                <div>
                  ...
                  <div className="relative flex items-center">
                    <input 
                      ...
                      className="w-full pl-5 pr-10 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-bold text-[#1E4A32] placeholder-gray-400 focus:outline-none opacity-50 cursor-not-allowed transition-all"
                    />
                    <User className="absolute right-4 w-5 h-5 text-gray-400" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative flex items-center">
                    <span className="absolute left-4 text-gray-500 font-bold text-sm opacity-50">+91</span>
                    <input 
                      type="tel"
                      maxLength={10}
                      value={mobile}
                    ...
                    />
                    <Smartphone className="absolute right-4 w-5 h-5 text-gray-400 opacity-50" />
                  </div>
                </div>
                ...
                >
                  Mobile Signup (Coming Soon)
                </button>

                <p className="text-center text-[11px] font-medium text-gray-500 pt-2">
                  Already have an account? <span onClick={() => router.push('/login')} className="font-bold text-[#307042] cursor-pointer hover:underline">Sign In</span>
                </p>
              </form>
            ) : (
